...
const AssignRoleToUser = ({ fetchData }) => {
  ...
  const handleFormAssignRole = async (e) => {
    e.preventDefault()
    const url = `${import.meta.env.VITE_EVENTS_API}/users/${userId}/`

    if (!userId || !roleId) return toast.error('Veuillez renseigner les deux champs', { duration: 3000 })
    ...
    try {
      ...
      if (res.success) {
        toast.success("nouveau role assignée avec succès", { duration: 2000 })
        setTimeout(() => {
  ...
  return (
    <div className="p-4 animate-fade-in">
      <h1 className="text-xl font-semibold text-gray-900 text-center mb-6">Assigner un nouveau role a un utilisateur</h1>
