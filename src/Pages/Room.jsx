    ...
    */
    ...
    const workShopExist = async (roomId) => {
        ...
        try {
            ...
            let workshopRes = await response.json();
            let workshop = workshopRes.result

            console.log("workshopRes.result", workshopRes.result)
            if (response.ok) {
                let data = {
    ...
    const updateWorkshop = async (roomId, data) => {
        ...
        try {
            ...
            // console.log("this is the response of the patch request :", workshop);
            // debugger
            if (response.ok) {
    ...
    const participantIsEligible = async (participants, userId) => {
        ...
        // Alerte
        window.alert("Vous n'êtes pas autorisé à rejoindre cette réunion. Vous allez être redirigé.");
    ...
    const createDevice = async () => {
        try {
            ...
            // Alerte
            window.alert("Navigateur non pris en charge, veuillez choisir au autre navigateur");
    ...
    const handleProjectClick = () => {
        const defaultAvatarUrl = avatarIcon;
        const defaultWorkshopAvatar = atelierEv;
        ...
        <body>
            <div class="relative w-full h-screen bg-gradient-to-r from-[#17161b] to-gray-800">
                <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-md bg-[rgba(0,0,0,0.5)] text-white w-full h-full">
                    <div class="flex justify-center items-center h-full">
                        <div class="text-center">
                            <div id="avatar" class="flex justify-center items-center mb-4"></div>
                            <div id="userName" class="px-2 pb-1 font-bold"></div>
                            <div id="description" class="px-2"></div>
                            <div id="talking" class="px-2"></div>
                        </div>
                    </div>
                </div>
            </div>
            <script>
                window.addEventListener("message", (event) => {
                    ...
                    const user = currentSpeakingUser?.[0] || {};
                    const avatarElement = document.getElementById("avatar");
                    const nameElement = document.getElementById("userName");
                    const descElement = document.getElementById("description");
                    const talkingElement = document.getElementById("talking");

                    nameElement.textContent = user.name || workshopData?.name || "";
                    descElement.textContent = user.description || "";
                    talkingElement.textContent = user.name ? "Entrain de parler..." : null;

                    const img = document.createElement("img");
                    img.src = user.avatar || workshopData?.photo || "${defaultWorkshopAvatar}";
                    img.className = "w-[150px] h-[150px] rounded-full";
                    img.onerror = function() {
                        this.src = user.name ? "${defaultAvatarUrl}" : "${defaultWorkshopAvatar}";
                    };

                    avatarElement.innerHTML = ""; // Clear previous
        ...
        <div className='relative w-full h-screen flex flex-col gap-3 p-4 bg-gradient-to-r from-[#17161b] to-gray-800'>
            ...
            <div className=' w-full rounded-lg bg-transparent h-[50px] flex items-center justify-between p-4 text-white'>
                ...
                <div className='flex items-center justify-center space-x-2'>
                    <img src={workshopData?.photo !== null ? workshopData?.photo : ""} alt="" className='h-10 w-10 flex items-center justify-center rounded-full' onError={(e) => { e.target.src = atelierEv }} />
                ...
                <div className='flex items-center space-x-3'>
                    <div className='flex items-center justify-center rounded-full overflow-hidden shadow-sm'>
                        <img src={userInfos.current?.avatar !== null ? userInfos.current?.avatar : ""} alt="" className='h-10 w-10 flex items-center justify-center rounded-full' onError={(e) => { e.target.src = avatarIcon }} />
                    </div>
