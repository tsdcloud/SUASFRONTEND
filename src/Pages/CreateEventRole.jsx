...
const CreateEventRole = ({ setCreate, fetchData }) => {

    document.title = "Créer le role dans les evenements"
    ...
    useEffect(() => {
        const fetchPermissions = async () => {
            const urlFile = `${import.meta.env.VITE_EVENTS_API}/permissions`
            try {
                const response = await handleFetch(urlFile)
                if(response.success){
    ...
    const handleFormCreateUsersRole = async (e) => {
        e.preventDefault()
        const url = `${import.meta.env.VITE_EVENTS_API}/eventparticipantroles/create`
        ...
        try {
            ...
            if (res.success) {
                toast.success("nouveau role évènement créee avec succès", { duration: 2000 })
                setTimeout(() => {
    ...
    return (
        <div className="p-4 animate-fade-in">
            <h1 className="text-xl font-semibold text-gray-900 text-center mb-6">Creer un nouveau role aux utilisateurs</h1>
            <div className="w-full max-w-xl mx-auto p-4 bg-white rounded-md ">
                ...
                {/* Formulaire */}
                <form onSubmit={handleFormCreateUsersRole} className="space-y-8">
