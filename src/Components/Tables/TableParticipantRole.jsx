...
export default function TableParticipantRole({ partRole }) {
  ...
  const filteredPartRoles = partRole.filter((pr) =>
    pr.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    pr.referenceNumber?.toString().includes(searchTerm)
  );

  // Pagination
  const totalPages = Math.ceil(filteredPartRoles.length / itemsPerPage);
  const paginatedPartRoles = filteredPartRoles.slice(
