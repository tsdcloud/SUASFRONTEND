...
export default function TablePermissions({ perms }) {
  ...
  const filteredPerms = perms.filter((perm) =>
    ...
    perm.referenceNumber?.toString().includes(searchTerm)
  );

  // Pagination
  const totalPages = Math.ceil(filteredPerms.length / itemsPerPage);
