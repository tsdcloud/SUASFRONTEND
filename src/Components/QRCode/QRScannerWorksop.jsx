...
const QRScannerWorksop = ({ onClose, workshopId }) => {
    ...
    const handleResult = (result, err) => {
        ...
        if (result?.text) {
            ...
            // const modUrl = url + `&workshopId=${workshopId}`
    ...
    const checkParticipation = async () => {
        let url = `${import.meta.env.VITE_EVENTS_API}/participants/?ownerId=${decodedUrl}&workshopId=${workshopId}`
        try {
            setIsLoading(true);
            ...
            if (response.success) {
                ...
                if (data.length !== 0 && data[0].isApproved) {
                    ...
                    <div className='relative w-72 h-52 overflow-hidden rounded-lg mb-2 flex flex-col justify-center items-center bg-gray-50 backdrop-blur-sm animate-fade-in'>
                        ...
                        <div className='text-green-500 font-semibold flex-1'>
                            <div className='text-xs text-black'>
                                Msr/Mme : {participantInfo?.name}
                            </div>
                            <div className='text-xs text-black'>
                                Role : <span className='text-green-500'>{participantInfo?.participantRole?.name}</span>
                            </div>
                        </div>
                    </div>
...
export default QRScannerWorksop;
